...
const testUsers = new SharedArray('test_users', function () {
  return Array.from({ length: 1000 }, (_, i) => ({
    // Use valid UUID format for user IDs
...
export default function () {
  ...
  group('Event Operations', function () {
    // Create event - use correct CreateEventCommand format
    const eventPayload = {
      user_id: testUser.id,
  ...
  group('User Operations', function () {
    // Create user (less frequently) - use correct format
    if (Math.random() < 0.1) { // 10% chance
      const userPayload = {
        name: `${testUser.name}_${__VU}_${__ITER}`,
    ...
    // Get user analytics - use correct endpoint
