...
/// This allows Memory, FileCache, and other cache implementations to exist
/// independently.
...
pub trait CacheTrait: Send + Sync {
    ...
    async fn clear(&mut self) -> CacheResult<()> {
        Err(CacheError::Unsupported(
            "Clear operation not supported by this cache implementation"
                .to_string(),
        ))
    }
}
...
pub trait CachePatternTrait: CacheTrait {
    ...
    async fn remove_pattern(&mut self, pattern: &str) -> CacheResult<u64>;

    /// Get all keys matching a pattern
    async fn keys(&mut self, pattern: &str) -> CacheResult<Vec<String>>;
}
