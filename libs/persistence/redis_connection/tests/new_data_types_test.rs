...
use redis_connection::{
    ...
    redis_key,
    core::type_bind::RedisTypeBind,
};
use test_utils::redis::TestRedisContainer;
use std::collections::HashSet;

// Test the new Redis data type macros
redis_key!(set UniqueUsers::<String> => "users:active:{}"[date: String]);
redis_key!(zset Leaderboard::<String> => "leaderboard:{}"[game_id: String]);
redis_key!(list RecentActivity::<String> => "activity:{}"[user_id: String]);
redis_key!(stream EventLog::<String> => "events:{}"[stream_id: String]);

async fn setup_test_redis() -> anyhow::Result<(TestRedisContainer, RedisConnectionManager)> {
...
async fn test_redis_set_operations() {
    ...
    let mut set = unique_users.bind_with(&mut conn, &"2024-12-14".to_string());
    ...
    let not_contains: bool = set.contains("user999".to_string()).await.unwrap();
    assert!(!not_contains);
...
async fn test_redis_sorted_set_operations() {
    ...
    let added: i32 = zset.add_with_score(100.0, "player1".to_string()).await.unwrap();
    assert_eq!(added, 1);
    
    let _: i32 = zset.add_with_score(85.0, "player2".to_string()).await.unwrap();
    let _: i32 = zset.add_with_score(120.0, "player3".to_string()).await.unwrap();
    ...
    let rev_rank: Option<usize> = zset.reverse_rank("player3".to_string()).await.unwrap();
    ...
    // Increment score
    let new_score: f64 = zset.increment_score("player2".to_string(), 20.0).await.unwrap();
...
async fn test_redis_stream_operations() {
    ...
    let mut stream = event_log.bind_with(&mut conn, &"app_events".to_string());

    // Add entries to stream
    let id1: String = stream.add_auto(&[("event", "user_login".to_string()), ("user_id", "123".to_string())]).await.unwrap();
    assert!(!id1.is_empty());
    
    let id2: String = stream.add_auto(&[("event", "page_view".to_string()), ("page", "/dashboard".to_string())]).await.unwrap();
    assert!(!id2.is_empty());
...
async fn test_redis_stream_consumer_groups() {
    ...
    let event_log = EventLog;
    let mut stream = event_log.bind_with(&mut conn, &"group_events".to_string());

    // Add some entries first
    stream.add_auto(&[("event", "test1".to_string())]).await.unwrap();
    stream.add_auto(&[("event", "test2".to_string())]).await.unwrap();

    // Create consumer group
    let created: String = stream.create_group("processors", "0").await.unwrap();
    ...
    let messages = stream.read_group("processors", "consumer1", ">").await.unwrap();
...
async fn test_backward_compatibility() {
    ...
    redis_key!(hash UserProfiles::<String> => "user:profile:{}"[user_id: String]);
    redis_key!(UserSession::<String> => "session:{}"[session_id: String]);

    let profiles = UserProfiles;
    let mut hash = profiles.bind_with(&mut conn, &"user_123".to_string());

    let set_result: i32 = hash.set("name", "John Doe".to_string()).await.unwrap();
    ...
    let mut session = session_key.bind_with(&mut conn, &"sess_456".to_string());

    let _: String = session.set("active_session_data".to_string()).await.unwrap();
...
async fn test_multiple_data_types_together() {
    ...
    // Track user activity in a list  
    {
        ...
        let _: i32 = activity_list.push_right("started_game".to_string()).await.unwrap();
        let _: i32 = activity_list.push_right("scored_points".to_string()).await.unwrap();
    ...
    // Update leaderboard with sorted set
    {
        ...
        let _: i32 = scores.add_with_score(150.0, user_id.clone()).await.unwrap();
        let user_score: Option<f64> = scores.score(user_id.clone()).await.unwrap();
    ...
    {
        let event_log = EventLog;
        let mut events = event_log.bind_with(&mut conn, &"game_events".to_string());
        events.add_auto(&[("user_id", user_id.clone()), ("action", "game_start".to_string())]).await.unwrap();
