...
async fn seed_users(pool: &PgPool, count: usize) -> Result<Vec<Uuid>> {
    ...
    // Use prepared statement for users
...
async fn worker_task(
    ...
    while let Ok(batch) = rx.recv() {
        ...
        // Build arrays for the prepared statement
        let mut ids = Vec::with_capacity(batch_size);
        let mut user_ids = Vec::with_capacity(batch_size);
        let mut type_ids = Vec::with_capacity(batch_size);
        let mut timestamps = Vec::with_capacity(batch_size);
        let mut metadatas = Vec::with_capacity(batch_size);

        for event in batch.events {
            ids.push(event.id);
            user_ids.push(event.user_id);
            type_ids.push(event.event_type_id);
            timestamps.push(
                chrono::DateTime::from_timestamp(event.timestamp, 0).unwrap(),
            );
            metadatas.push(sqlx::types::Json(event.metadata));
        }
